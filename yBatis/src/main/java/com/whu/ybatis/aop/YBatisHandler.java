package com.whu.ybatis.aop;

import com.whu.ybatis.annotation.Arguments;
import com.whu.ybatis.annotation.ResultType;
import com.whu.ybatis.annotation.Sql;
import com.whu.ybatis.aspect.YBatisInterceptor;
import com.whu.ybatis.constants.YBatisConstants;
import com.whu.ybatis.pojo.SimpleDaoPage;
import com.whu.ybatis.springMapper.rowMapper.YBatisColumnMapRowMapper;
import com.whu.ybatis.springMapper.rowMapper.YBatisColumnOriginalMapRowMapper;
import com.whu.ybatis.util.FreemarkerParseFactory;
import com.whu.ybatis.util.ParameterNameUtils;
import com.whu.ybatis.util.YBatisUtil;
import ognl.Ognl;
...
public class YBatisHandler implements InvocationHandler {
...
//		Map<String, Object> rs = new HashMap<String, Object>();
        ...
        if (showSql) {
            System.out.println("YBatis-SQL:\n\n" + executeSql);
            logger.info("YBatis-SQL:\n\n" + executeSql);
        }
        return returnObj;
    }
    ...
    private static boolean checkActiveKey(String methodName) {
        String keys[] = YBatisConstants.INF_METHOD_ACTIVE.split(",");
        for (String s : keys) {
    ...
    private static boolean checkBatchKey(String methodName) {
        String keys[] = YBatisConstants.INF_METHOD_BATCH.split(",");
        for (String s : keys) {
            if (methodName.startsWith(s))
                return true;
        }
    ...
    private RowMapper<Map<String, Object>> getColumnMapRowMapper() {
        if (getKeyType().equalsIgnoreCase(LOWER_KEY)) {
            return new YBatisColumnMapRowMapper();
        } else if (getKeyType().equalsIgnoreCase(UPPER_KEY)) {
            return new ColumnMapRowMapper();
        } else {
            return new YBatisColumnOriginalMapRowMapper();
        }
    }
        ...
        if (checkActiveKey(methodName) || checkActiveSql(executeSql)) {
            ...
            if (returnType.isPrimitive()) {
                ...
                if (page != 0 && rows != 0) {
                    ...
                    executeSql = YBatisUtil.createPageSql(dbType, executeSql, page, rows);
                }
            ...
            } else if (YBatisUtil.isWrapClass(returnType)) {
                if (paramMap != null) {
    ...
    private String installDaoMetaData(SimpleDaoPage pageSetting, Method method, Map<String, Object> sqlParamsMap, Object[] args) throws Exception {
        ...
        if(yBatisInterceptor!=null && args!= null && args.length==1){
            ...
            if(methodName.startsWith("insert")){
                if(yBatisInterceptor!=null){
                    yBatisInterceptor.onInsert(fields, obj);
                }
            }
            if(methodName.startsWith("update")){
                if(yBatisInterceptor!=null){
                    yBatisInterceptor.onUpdate(fields, obj);
                }
            }
            //reflect(obj);
        }
